...
function ret = get_like_alt_h_gIP(DATA,x,Time,Conc,NR,NC,NT,s,cmd)
    theta = x;
    c = theta(end)';
%     ti = Time(2) - Time(1);
    ...
    p     = Theta(:,1)';  % These are 2 parameters that guess the initial proportion of dosage 8 and dosage 16.
    Theta(:,1) = [];
    b    = Theta(:,1)';
    ...
    for i = 1:NC
        ...
        MM      = [];
        for r = 1:NR
            if Conc(i) == 8   % As we observe at time 24 the initial proportion is most likely not longer be 0 on Conc = 8 and 16
                init = DATA(1,i,r)*[p(1),1-p(1)];
            elseif Conc(i) == 16
                init = DATA(1,i,r)*[p(2),1-p(2)];
            else
                init = DATA(1,i,r)*[0,1];
            end
            Meani   = zeros(1,NT-1);
