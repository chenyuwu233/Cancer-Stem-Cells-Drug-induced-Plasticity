...
function ret = get_like_VIP_h(DATA,x,Time,Conc,NR,NC,NT,s,cmd,IP_vec)
    theta = x;
    c = theta(end)';
    ...
    init = [mean(squeeze(DATA(1,:,:)),2,"omitnan").*IP_vec,mean(squeeze(DATA(1,:,:)),2,"omitnan").*(1-IP_vec)];
