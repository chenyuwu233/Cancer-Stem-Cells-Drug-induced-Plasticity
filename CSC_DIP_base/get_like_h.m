...
%  
%  Input:
%  - DATA: NR x NC x NT tensor that records all the data
...
function ret = get_like_h(DATA,x,Time,Conc,NR,NC,NT,s,cmd)
    theta = x;
    c = theta(end)';
    ti = Time(2) - Time(1);
%     if ti ~= Time(3) - Time(2)
...
%     end
    ...
    theta(end) = [];
    [Theta,p] = coeff_reshape_h(theta,cmd,'Like',s);
    ...
    ret = 0;
    switch cmd
        case {'Linear_switching','Single_Broad','Hill2_switching_death','Single_Broad_d'}
            for i = 1:NC
...
%                 MM     = repmat(Meani,NR,1);
        ...
        case {'Hill2_switching_Hill2_death_i_EP','Hill2_death_Hill2_death_i_EP',...
                'Multi_Broad_EP','Multi_Selective_EP','Hill2_death_2_i_EP'}
            for i = 1:NC
                Ai      = Drug_A(Theta,Conc(i,:),cmd);
                % Mean
                Mi      = get_Mean(Ai,Time); % s x s x NT tensor
